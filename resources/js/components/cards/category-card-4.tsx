...
const CourseCard4 = ({ enrollment, className }: Props) => {
   ...
   const batchNo = (course as any).batch_no ?? null;
   const displayTitle = batchNo ? `${course.title} (Batch-${batchNo})` : course.title;
   ...
   return (
      <Card className={cn('overflow-hidden shadow-sm', className)}>
         ...
         <CardContent className="p-4">
            ...
            >
               <p className="hover:text-secondary-foreground text-sm font-semibold">{displayTitle}</p>
            </Link>
         </CardContent>
      </Card>
   );
